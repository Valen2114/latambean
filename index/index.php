...
<body>
  <div class="container">
    ...
    <div class="containeres">
        ...
        $result_total = mysqli_query($conexion, "SELECT COUNT(*) as total FROM inventario WHERE id_categoria = 8 ");
        $total_productos = mysqli_fetch_assoc($result_total)['total'];

        // Calcular el número de contenedores necesarios
        $num_contenedores = ceil($total_productos / 8);

        // Generar los botones de navegación
        for ($i = 1; $i <= $num_contenedores; $i++) {
            echo '<label class="checkbox">
                    <input type="radio" name="checkboxGroup" ' . ($i === 1 ? 'checked="checked"' : '') . ' data-target="info-' . $i . '">
    ...
    <?php
    // Generar los contenedores de información
    for ($i = 1; $i <= $num_contenedores; $i++) {
        echo '<div class="info-container" data-target="info-' . $i . '">';
        echo '<div id="promociones" class="content">';
        ...
        $result = mysqli_query($conexion, "SELECT * FROM inventario WHERE id_categoria = 8 LIMIT 8 OFFSET $offset");
    ...
    <p class="ofertas">
        <br><a href="../productos/productos.php">Ver más productos</a>
    </p>
</section>
